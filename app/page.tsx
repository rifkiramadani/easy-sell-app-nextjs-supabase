    ...
    <>
      <div className="container mx-auto flex sm:flex-col sm:items-center sm:justify-center px-5">
        <div className="flex flex-col md:flex-row sm:py-10 px-5">
          ...
          <div className="flex flex-col ms-2 py-3 sm:gap-5 sm:flex-row">
            {
              products.map(product => (
                <Card key={product.id} {...product} /> //{...product} => spreading semua value yang ada di array ke component card
              ))
            }
          </div>
        </div>
        <div className="py-5 px-10 sm:py-10 sm:px-5">
          <span className={`text-4xl sm:px-5 font-bold ${ubuntu.className}`}>All Products</span>
